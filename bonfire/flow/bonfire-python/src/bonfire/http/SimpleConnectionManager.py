...
class HttplibResponseWrapper(object):
	...
	def close(self):
		try:
			...
			logger.exception("Error closing response")
		finally:
			self.__connection.close()

class SimpleConnectionManager(object):
	def __init__(self, host, port, certfile = None, keyfile = None, force_ssl = False, *args, **kw):
		...
		logger.debug("Creating SimpleConnectionManager for %s:%s", host, port)
