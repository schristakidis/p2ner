...
net1 = broker.occi("POST", experiment + "/networks", toprettyxml(newres), validate = False)
...
NETWORKS = ["/locations/be-ibbt/networks/1", net1]
...
STORAGE = "/locations/be-ibbt/storages/1"
NETWORKS = ["/locations/be-ibbt/networks/1", net1]
newres = ComputeResource(name = "Client", location = "/locations/be-ibbt", experiment = experiment)
newres.instance_type = "Large-EN"
newres.disk = [ Disk(STORAGE) ]
newres.disk[0].type = "OS"
newres.disk[0].target = "hda"
...
print "waiting for hosts to come up"
'''
while True:
