...
class UDPsender(threading.Thread):
    ...
    def setW(self):
        ...
        self.dREF=0    
        for p in peers:
            self.dREF += PeerRtt[p]['ref']-PeerRtt[p]['min']

        self.dREF = self.dREF/len(peers)
...
class Producer(threading.Thread):
    ...
    def __init__(self, nreceivers=4):
        threading.Thread.__init__(self)
        self.nb = 7
...
class ACKreceiver(threading.Thread):
    ...
    def run(self):
        global History,LastAck,GTsend,RTT,peers
        while True:
            ...
            RTT[peers.index(peer)] = self.peers[peer]['av']-minRtt
            if not PeerRtt[peer].has_key('meanmax'):
                er=minRtt+GTsend
            else:
                er=PeerRtt[peer]['max']
