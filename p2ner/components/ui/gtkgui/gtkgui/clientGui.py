...
class clientGui(UI):
    ...
    def subscribeStream(self,id,ip,port,remote=False):
        ...
        for k,v in st.items():
            if v.has_key('optional') and not v['optional'] and not v['value']:
                self.displayError('you must set the values for the %s from the preferences menu'%output)
                return
    ...
    def registerStreamSettings(self,settings):
        ...
        for k,v in st.items():
            if v.has_key('optional') and not v['optional'] and not v['value']:
                self.displayError('you must set the values for the %s from the preferences menu'%output['default'])
                return
        out={}
